...
DATA_DIR = "/Users/pinki/academics/CV/calibration/"
DEBUG_DIR = "/Users/pinki/academics/CV/calibration/debug/"
PATTERN_SIZE = (7, 6)
SQUARE_SIZE = 1.0 #
image_points = []
object_points = []
    ...
    for (path, each) in get_camera_images(): #images:
        ...
        if ret:
            ...
            if visualize:
                ...
                gray = cv2.cvtColor(ec ,cv2.COLOR_BGR2GRAY)
                print(len(ec.shape))
                print("Yooooo")
                cv2.drawChessboardCorners(ec, PATTERN_SIZE, corners, ret)
                print(cv2.calibrateCamera(object_points, image_points, gray.shape[::-1],None,None))
